...
public class Command_ShiftDriveTarget implements BaseCommand {
    @Override
    public CommandResult runCommand(@NotNull String args, @NotNull BaseCommand.CommandContext context) {
        ...
        if (args.isEmpty()) {
            ...
            if ("hyperspace".equalsIgnoreCase(args)) {
                ...
                ShiftDrive_AbilityPlugin shiftDrivePlugin = ItCameFromBeyond.Global.getPlayerShiftDrivePlugin();
                if (shiftDrivePlugin == null) {
                    Console.showMessage("Player fleet doesn't have the Shift Drive ability!");
                    return CommandResult.ERROR;
                }
                if (system.getStar() == null) {
                    Console.showMessage("Target system doesn't have a star!");
                    return CommandResult.ERROR;
                }
                shiftDrivePlugin.getImpl().setTarget(system.getStar());
                return CommandResult.SUCCESS;
            }
        }
    }
